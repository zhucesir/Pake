(() => {
  console.log('>>> [Pake event.js v3.5] 上帝脚本（纯粹净秒去广告 + 隐形雷达版）已就位！');
  ...
  function skipAds() {
    ...
    if (isAdPlaying) {
      ...
      if (media) {
        ...
        if (isFinite(media.duration) && media.duration > 0 && media.duration <= 16) {
  ...
  const UniversalSeed = {
    locate(query) {
      ...
      const isSelector = typeof query === "string" && (
        query.startsWith(".") || query.startsWith("#") || query.startsWith("[") || query.includes(" ")
      );
      let candidates = [];
      if (isSelector) {
        try { candidates = Array.from(document.querySelectorAll(query)); } catch (e) {}
      } else {

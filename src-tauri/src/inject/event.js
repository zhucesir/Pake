const shortcuts = {
  ...
  L: () => copyCurrentPageUrl(),
  w: () => window.__TAURI__?.window?.getCurrentWindow()?.minimize(),
  W: () => window.__TAURI__?.window?.getCurrentWindow()?.minimize(),
  h: () => returnToHomePage(),
  H: () => returnToHomePage(),
  ...
  Home: () => scrollTo(0, 0),
  End: () => scrollTo(0, document.body.scrollHeight),
  F12: () => openDevTools(),
  i: () => openDevTools(),
  I: () => openDevTools(),
};
...
function returnToHomePage() {
  const homeUrl = window.pakeConfig?.url || window.location.origin;
  if (homeUrl) {
...
// ================= YouTube 去广告(Anti-Tracking & Ad-Skip) 模块 =================
(() => {
  ...
  setInterval(() => {
    ...
    // Universal skip button selectors (YouTube & YouTube Music)
    const skipSelectors = [
      ".ytp-ad-skip-button",
    ...
    skipSelectors.forEach((sel) => {
      document.querySelectorAll(sel).forEach((btn) => syntheticClick(btn));
    });
